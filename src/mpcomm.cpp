...
void MPComm::Impl::workerThreadLoop(size_t worker_id, int numa_id, int cpu_id) {
    ...
    while (worker_running_.load(std::memory_order_relaxed)) {
        ...
        // Iterate all active contexts round-robin, posting chunks until NICs are full
        // or all contexts have posted all their chunks.
...
uintptr_t MPComm::Impl::mapDRAMtoGPU(void *host_addr, size_t length) {
    ...
    (void)host_addr;
    (void)length;
    MPCOMM_LOG_ERROR("MPComm: mapDRAMtoGPU: CUDA support not compiled (USE_CUDA=OFF)\n");
    return 0;
#endif
}
...
#else
    (void)host_addr;
    MPCOMM_LOG_ERROR("MPComm: unmapDRAMfromGPU: CUDA support not compiled (USE_CUDA=OFF)\n");
    return MPCOMM_ERR_DEVICE;
#endif
}
...
int MPComm::Impl::tmaScatter(void *gpu_src, const long *indices,
    ...
    (void)num_blocks; (void)block_size; (void)max_sm_count;
    MPCOMM_LOG_ERROR("MPComm: tmaScatter: CUDA support not compiled (USE_CUDA=OFF)\n");
    return MPCOMM_ERR_DEVICE;
#endif
}
